...
def test(c):
    ...
    c.run('docker run --rm -v $(pwd)/code:/code -v /etc/hosts:/etc/hosts selenium_scrap python3 /code/test.py')

@task
def build(c):
    """Builds the docker image to work with"""
    c.run('docker build -t selenium_scrap .')

@task
def run(c):
    """Runs a container, mounting ./code and local /etc/hosts, runs ./code/main.py and removes the container when done"""
    c.run('docker run --rm -v $(pwd)/code:/code -v /etc/hosts:/etc/hosts selenium_scrap python3 /code/main.py')
