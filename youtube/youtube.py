...
def login():
    ...
    if os.path.exists('cookies.pkl'):
        ...
        try:
            ...
            email_input.send_keys(config.youtube_username)
            pass_input.send_keys(config.youtube_password)
            form.submit()
...
def search(name, location_id, pages=10):
    pages = min(100, pages)
    url = "https://www.youtube.com/search/str/{search_term}/users-named/{location_id}/residents/present/intersect"
#    url = "https://www.youtube.com/search/results/?init=quick&q={search_term}&tas=0.4872438906449996"
